...
def process_phobos(arguments):
        ...
        phobos_arguments = [phobos_path, "/dev/stdin", "/dev/stdout", "--outputFormat 1", "--reportUnit 1", "--printRepeatSeqMode 2", "--minUnitLen " + str(min_unit_size), "--maxUnitLen " + str(max_unit_size)]
...
def parse_parameters():
        ...
        for subparser in [parser_processrepeats, parser_summarizecounts]:
                if subparser == parser_processrepeats:
                        ...
                        subparser.add_argument("outputDirectory", type=str, help="Directory where the output will be written.")
                        subparser.add_argument("repeatCallerPath", type=str, help="Path to Phobos or TRF executable.")
                        subparser.add_argument("--outputPrefix", type=str, default="", dest="output_prefix", help="Prefix of output files, prefix will be taken from input file, if not set (default: %(default)s).")
                        subparser.add_argument("--outputType", type=str, default="c", dest="output_type", help="Output to generate, countstable.txt (c), repeatinfo.txt (i), repeatinfo.txt.gz (g), concatenate the letters for multiple outputs, e.g. 'ci' (countstable.txt and repeatinfo.txt)  (default: %(default)s).")
                        ...
                        subparser.add_argument("--minUnitSize", type=int, default=1, dest="min_unit_size", help="Minimum repeat unit size for a repeat to be considered (default: %(default)s).")
                        subparser.add_argument("--maxUnitSize", type=int, default=6, dest="max_unit_size", help="Maximum repeat unit size for a repeat to be considered (default: %(default)s).")
