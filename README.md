...
Countstable files are generated by default by the the processrepeats function and cotain the computed repeat counts per group for one sample. They always consist of two tab-separated columns with the first column being the group and the second column being the counts of the respectiv group.
...
```
### countmatrix.txt files
```
Repeat_group  strcount_out/test.countstable.txt  strcount_out/test.countstable.txt  strcount_out/test.countstable.txt  strcount_out/test.countstable.txt
ACCG <perfection:[100.0,100.0] length:[0.0,40.0)>  1  1  1  1
AG <perfection:[100.0,100.0] length:[0.0,40.0)>  2  2  2  2
AT <perfection:[0.0,100.0) length:[40.0,80.0)>  1  1  1  1
AT <perfection:[100.0,100.0] length:[0.0,40.0)>  1  1  1  1
CTT <perfection:[100.0,100.0] length:[0.0,40.0)>  1  1  1  1
```
